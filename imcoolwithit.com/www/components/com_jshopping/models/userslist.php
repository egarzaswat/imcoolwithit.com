...
class jshopUsersList{
    ...
    function getRejectUsers(){
        $db = JFactory::getDBO();
        $query = "SELECT `id_user_guest`, `date` FROM `#__rejected_users` where `id_user_active` = " . JSFactory::getUser()->user_id . ' ORDER BY date ASC';
        $db->setQuery($query);
        $result = array();
    ...
    function usersList($searchParams, $start = 0, $limit = 0, $fields_arr = array(), $info_fields_array = array(), $quickConnect = false){
        ...
        foreach($result_all_users as $key => $value){
            if(in_array($value->user_id, $reject_users)){
                $result_reject_users[array_search($value->user_id, $reject_users)] = $value;
...
//                array_push($result_reject_users, $value);
        ...
        ksort($result_reject_users);
        reset($result_reject_users);
        $result_reject_users = array_values($result_reject_users);
