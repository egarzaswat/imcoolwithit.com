...
$notifications = $m_not->getLincUpNotes();
...
foreach($user_not as $value){
    $message = $m_not->getMessage('Linc Up Notification', $value['user_id'], $value['count']);
    $m_not->sendNotification($message, $value['email'], 'Link Up Notification');
}
