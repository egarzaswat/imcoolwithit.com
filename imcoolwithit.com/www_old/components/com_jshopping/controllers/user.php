...
//        $this->setRedirect(SEFLink("index.php?option=com_jshopping&controller=user&task=editaccount",0,1,$jshopConfig->use_ssl), $message);
    ...
    function myaccount(){
        ...
        $modelUser = JSFactory::getModel('user', 'jshop');
        $adv_user = $modelUser->getDataUser(JSFactory::getUser()->user_id, array('user_id', 'u_name', 'birthday', 'city', 'state', 'photosite', 'block', 'user_reviews', 'looking_for', 'age_look_from', 'age_look_to', 'distance', 'zip', 'longitude', 'latitude', 'sex', 'user_reviews', 'last_visit'), true);
...
//        $view->assign('menu',$menu);
    ...
    function show_private_pictures(){
        ...
        $isFriend = (!$modelFriends->getIsFrieds($user_id) && !$modelFriends->getIsAccept($user_id)) ? false : true;
