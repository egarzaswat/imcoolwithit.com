...
class JshoppingControllerAttributes extends JControllerLegacy{
    ...
    function display($cachable = false, $urlparams = false){
        ...
        if($_GET['category'] == null || !isset($_GET['category']) || $_GET['category'] == 0){
            $category = null;
        } else {
            $category = array(
                0 => (int)$_GET['category']
        ...
        $categories = $_categories->getTreeAllCategories();

    	$attributes = JSFactory::getModel("attribut");
    	$attributesvalue = JSFactory::getModel("attributValue");
        $rows = $attributes->getAllAttributes(0, $category, $filter_order, $filter_order_Dir);
    ...
    function edit() {
        ...
        if (!$attribut->independent) $attribut->independent = 1;
        ...
        if (!isset($attribut->allcats)) $attribut->allcats = 0;
...
/*        $mgroups = JSFactory::getModel("attributesgroups");
	...
	function remove() {
		...
		foreach ($cid as $key => $value) {
            ...
            $value = intval($value);
            $query = "DELETE FROM `#__jshopping_products_attr2` WHERE `attr_id` = '".$db->escape($value)."'";
            $db->setQuery($query);
            $db->query();
