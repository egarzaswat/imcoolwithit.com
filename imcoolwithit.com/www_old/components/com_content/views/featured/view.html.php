...
class ContentViewFeatured extends JViewLegacy
{
	...
	protected function _prepareDocument()
	{
        ...
        if($menus->getDefault() == $menus->getActive()){
            $db = JFactory::getDBO();
            $query = "SELECT * FROM `#__meta_data` WHERE `page` = 'home_site'";
            $db->setQuery($query);
            $meta_data = $db->loadObjectList();

            $this->document->setTitle($meta_data[0]->title);
            $this->document->setDescription($meta_data[0]->description);
            $this->document->setMetadata('keywords', $meta_data[0]->keywords);
        } else {
            if (empty($title))
            {
