...
class jshopBookmarks{
    ...
    function getCountBookmarks($userId){
        ...
        $query = " SELECT COUNT(B.`reciper`) FROM `#__bookmarks` AS B LEFT JOIN `#__jshopping_users` AS U ON B.reciper = U.user_id WHERE `sender` = ".$userId." and U.block = 0 ORDER BY `date` DESC ";
        $db->setQuery($query);
    ...
    function getAllMyBookmarks($userId, $start = 0, $limit = 0){
        ...
        $query = " SELECT B.date, B.reciper, U.user_id, U.u_name, U.birthday, U.photosite, U.longitude, U.latitude, U.sex, U.last_visit, U.block, I.height, I.body, I.status FROM `#__bookmarks` AS B LEFT JOIN `#__jshopping_users` AS U ON B.reciper = U.user_id LEFT JOIN `#__user_info` AS I ON U.user_id = I.user_id WHERE B.sender = ".$userId." and U.block = 0 ORDER BY B.date DESC ";
