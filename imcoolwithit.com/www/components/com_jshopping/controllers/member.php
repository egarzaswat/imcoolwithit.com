...
class JshoppingControllerMember extends JControllerLegacy{

    function __construct($config = array()){
        parent::__construct( $config );
        JPluginHelper::importPlugin('jshoppingcheckout');
        JPluginHelper::importPlugin('jshoppingorder');
        JDispatcher::getInstance()->trigger('onConstructJshoppingControllerMember', array(&$this));
    }
    ...
    function login(){
        ...
        $view->assign('href_register', SEFLink('index.php?option=com_jshopping&controller=member&task=register',1,0, $jshopConfig->use_ssl));
        ...
        $view->assign('urlcheckdata', SEFLink("index.php?option=com_jshopping&controller=member&task=check_user_exist_ajax&ajax=1", 1, 1, $jshopConfig->use_ssl));
    ...
    function loginsave(){
        ...
        if ((!JError::isError($error)) && ($error !== FALSE)){
            ...
            $mainframe->redirect( SEFLink('index.php?option=com_jshopping&controller=member&task=login&return='.base64_encode($return),0,1,$jshopConfig->use_ssl) );
        }
    }
    ...
    function register(){
        ...
        $view->assign('live_path', JURI::base());
        $view->assign('urlcheckdata', SEFLink("index.php?option=com_jshopping&controller=member&task=check_user_exist_ajax&ajax=1",1,1));
        $dispatcher->trigger('onBeforeDisplayRegisterView', array(&$view));
        $view->display();
    }
    ...
    function registersave(){
        ...
        if (!$row->check("register")){
            ...
            $this->setRedirect(SEFLink("index.php?option=com_jshopping&controller=member&task=register&lrd=1",1,1, $jshopConfig->use_ssl));
            return 0;
        }
        if ($post["u_name"]==""){
        ...
        if (!$user->save()){
            ...
            $this->setRedirect(SEFLink("index.php?option=com_jshopping&controller=member&task=register",1,1,$jshopConfig->use_ssl));
            return 0;
        }
        ...
        if ($useractivation == 2){
            ...
            $data['activate'] = $base.JRoute::_('index.php?option=com_jshopping&controller=member&task=activate&token='.$data['activation'], false);
            ...
            if ($sendpassword) {
                $emailBody = JText::sprintf(
                    ...
                    $data['name'],
                    $data['sitename'],
                    $data['siteurl'].'index.php?option=com_jshopping&controller=member&task=activate&token='.$data['activation'],
                    $data['siteurl'],
                    $data['username'],
                    $data['password_clear']
                );
        ...
        $return = SEFLink("index.php?option=com_jshopping&controller=member&task=login",1,1,$jshopConfig->use_ssl);
    ...
    public function activate(){
        ...
        if ($useractivation == 0){
            ...
            $this->setRedirect(SEFLink("index.php?option=com_jshopping&controller=member&task=login",0,1,$jshopConfig->use_ssl));
        }elseif ($useractivation == 1){
...
//        $this->setRedirect(SEFLink("index.php?option=com_jshopping&controller=user&task=editaccount",0,1,$jshopConfig->use_ssl), $message);
    ...
    function orders(){
        ...
        foreach($orders as $key=>$value){
            $orders[$key]->order_href = SEFLink('index.php?option=com_jshopping&controller=member&task=order&order_id='.$value->order_id,0,0,$jshopConfig->use_ssl);
    ...
    function order(){
        ...
        appendPathWay(_JSHOP_MY_ORDERS, SEFLink('index.php?option=com_jshopping&controller=member&task=orders',0,0,$jshopConfig->use_ssl));
    ...
    function cancelorder(){
        ...
        if ($order->order_status==$status || in_array($order->order_status, $jshopConfig->payment_status_disable_cancel_client)){
            $this->setRedirect(SEFLink("index.php?option=com_jshopping&controller=member&task=order&order_id=".$order_id,0,1,$jshopConfig->use_ssl));
            return 0;
        }
        ...
        $this->setRedirect(SEFLink("index.php?option=com_jshopping&controller=member&task=order&order_id=".$order_id,0,1,$jshopConfig->use_ssl), _JSHOP_ORDER_CANCELED);
    }
    ...
    function profile(){
        ...
        $userData->looking_for = (int)$userData->looking_for;

        if(is_null($userData->looking_for) || $userData->looking_for == 0){
            $userData->look = JText::_('UNKNOWN');
        } else {
            if($userData->looking_for == 2){
                $userData->look = JText::_('MALE');
            }
            if($userData->looking_for == 1){
                $userData->look = JText::_('FEMALE');
            }
            if($userData->looking_for == 3){
                $userData->look = JText::_('EITHER');
            }
        }
