...
class JUser extends JObject
{
	...
	public function save($updateOnly = false)
	{
        ...
        $this->updateUserShop($this);

		return $result;
	}

    function updateUserShop($user) {
        $db = JFactory::getDBO();
        if($user->block == null){
            $user->block = 0;
        }
        $query = "UPDATE `#__jshopping_users` SET `u_name` = '" . $user->username . "', `email` = '" . $this->email . "', `block` = " . $user->block . " WHERE `user_id` = " . $user->id;
