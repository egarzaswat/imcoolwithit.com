		...
		if ($this->_mode == JROUTER_MODE_SEF && $route)
		{
			...
			if ($app->get('sef_rewrite'))
			{
				...
				else
				{
                    ...
                    $conf = new JConfig();
                    $arr_controllers = explode(' ', $conf->paths);
                    $arr_replease = array();
                    foreach($arr_controllers as $key => $value){
//                        $arr_controllers[$key] = $value . '/';
                        $arr_replease[$key] = 'index.php/component/jshopping/' . $value;
                    }

					$route = str_replace($arr_replease, $arr_controllers, $route);
	...
	protected function parseSefRoute(&$uri)
	{
		...
		// Parse the application route
        $conf = new JConfig();
        $paths = explode(' ', $conf->paths);
        $segments = explode('/', $route);

        if(in_array($segments[0], $paths)){
            $tmp = $segments;
            $segments = array();
            array_push($segments, 'component');
            array_push($segments, 'jshopping');
            for($in=0; $in<count($tmp); $in++){
                array_push($segments, $tmp[$in]);
            }
        }
