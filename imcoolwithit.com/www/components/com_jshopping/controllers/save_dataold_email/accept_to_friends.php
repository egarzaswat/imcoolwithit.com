...
function acceptUserToFriends($myId, $userId){
    $config=new JConfig();
    ...
    if ($count_my_tokens - $count_tokens >=0) {
        ...
        if(count($result_rec) >= 1){
            ...
            $query = "INSERT "
                   . "INTO {$db->quoteName('#__messages_accept_tokens')} "
                   . "SET `sender` = {$myId}, `reciper` = {$db->escape($userId)}, `date` = '" . date("Y-m-d H:i:s") . "', `confirmation` = 1";
            $db->setQuery($query);
            $db->query();
...
if(isset($_POST['user_id'])){
    ...
    echo acceptUserToFriends($current_user, $user_id);
}